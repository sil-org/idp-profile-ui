...
import { mfa as mfaStore, retrieve as retrieveMfa } from '@/global/mfa'
...
export default {
  ...
  async init() {
    ...
    allSteps.forEach((step, index) => {
      const isRelevant = step.isRelevant(user, recoveryMethods.alternates, mfaStore)
...
const password = {
  ...
  isRelevant(u) {
    return isRequested(this.paths) || u.isNew()
  },
}
...
const recovery = {
  ...
  isRelevant(u, alternates) {
    return u.auth_type === 'login' && (isRequested(this.paths) || alternates.filter(isAlternate).length < 1)
  },
}
...
const totp = {
  ...
  isRelevant(u, _, mfa) {
    return u.auth_type === 'login' && (isRequested(this.paths) || (!mfa.totp?.id && mfa.numVerified < minVerified))
  },
}
...
const securityKeyStep = {
  ...
  isRelevant(u, _, mfa) {
    ...
    return u.auth_type === 'login' && (isRequested(this.paths) || numberOfKeys === 0 || mfa.numVerified < minVerified)
  },
}
...
const backupCodesStep = {
  ...
  isRelevant(u, _, mfa) {
    return u.auth_type === 'login' && (isRequested(this.paths) || mfa.numVerified < minVerified)
  },
}
